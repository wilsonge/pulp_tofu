...
if [[ "$GITHUB_WORKFLOW" == "Tofu changelog update" ]]; then
  # Do not build bindings docs on changelog update
  exit
fi

mkdir -p ../tofu-bindings
tar -xvf tofu-python-client-docs.tar --directory ../tofu-bindings
pushd ../tofu-bindings

# publish to docs.pulpproject.org/pulp_tofu_client
rsync -avzh site/ doc_builder_pulp_tofu@docs.pulpproject.org:/var/www/docs.pulpproject.org/pulp_tofu_client/

# publish to docs.pulpproject.org/pulp_tofu_client/en/{release}
rsync -avzh site/ doc_builder_pulp_tofu@docs.pulpproject.org:/var/www/docs.pulpproject.org/pulp_tofu_client/en/"$2"
popd
