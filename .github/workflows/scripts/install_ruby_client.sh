...
then
  ...
  USE_LOCAL_API_JSON=1 ./generate.sh pulp_tofu ruby "$VERSION"
else
  ./generate.sh pulp_tofu ruby "$VERSION"
fi

pushd pulp_tofu-client
gem build pulp_tofu_client
gem install --both "./pulp_tofu_client-$VERSION.gem"
tar cvf ../../pulp_tofu/tofu-ruby-client.tar "./pulp_tofu_client-$VERSION.gem"
popd
popd
