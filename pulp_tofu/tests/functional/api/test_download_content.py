...
from pulp_tofu.tests.functional.constants import TOFU_FIXTURE_URL
from pulp_tofu.tests.functional.utils import (
    gen_tofu_client,
    get_tofu_content_paths,
    gen_tofu_remote,
)
...
# Implement tests.functional.utils.get_tofu_content_unit_paths(),
...
class DownloadContentTestCase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        * `Pulp Smash #872 <https://github.com/pulp/pulp-smash/issues/872>`_
        ...
        client = gen_tofu_client()
        repo_api = RepositoriesTofuApi(client)
        remote_api = RemotesTofuApi(client)
        publications = PublicationsTofuApi(client)
        distributions = DistributionsTofuApi(client)
        ...
        publish_data = TofuTofuPublication(repository=repo.pulp_href)
        ...
        unit_paths = [choice(paths) for paths in get_tofu_content_paths(repo.to_dict()).values()]
        fixtures_hashes = [
            hashlib.sha256(utils.http_get(urljoin(TOFU_FIXTURE_URL, unit_path[0]))).hexdigest()
